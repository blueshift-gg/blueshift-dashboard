...
<ArticleSection name="Các tài khoản cần thiết" id="required-accounts" level="h2" />- `borrower`:
Người dùng đã yêu cầu flash loan. Họ cung cấp lamports để tạo tài khoản `loan`. Phải có thể thay đổi
(mutable). - `loan`: Tài khoản tạm thời được sử dụng để lưu trữ `protocol_token_account` và
`balance` cuối cùng được yêu cầu. Phải có thể thay đổi (mutable) vì nó sẽ được đóng ở cuối lệnh.
