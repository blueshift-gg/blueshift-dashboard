...
<ArticleSection name="Akun yang Diperlukan" id="required-accounts" level="h2" />- `borrower`:
Pengguna yang meminta pinjaman kilat. Mereka menyediakan lamports untuk membuat akun `loan`. Harus
dapat diubah (mutable). - `loan`: Akun sementara yang digunakan untuk menyimpan
`protocol_token_account` dan `balance` akhir yang diperlukan. Harus dapat diubah (mutable) karena
akan ditutup pada akhir instruksi.
