...
<ArticleSection name="Erforderliche Konten" id="required-accounts" level="h2" />- `borrower`: Der
Benutzer, der den Flash-Loan angefordert hat. Er stellt die Lamports für die Erstellung des
`loan`Kontos bereit. Muss veränderbar sein. - `loan`: Das temporäre Konto, das verwendet wird, um
die `protocol_token_account` und die erforderliche finale `balance` zu speichern. Muss veränderbar
sein, da es am Ende der Anweisung geschlossen wird.
