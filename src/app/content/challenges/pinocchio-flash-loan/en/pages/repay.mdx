...
<ArticleSection name="Required Accounts" id="required-accounts" level="h2" />- `borrower`: The user
who requested the flash loan. They provide the lamports for creating the `loan` account. Must be
mutable. - `loan`: The temporary account used to store the `protocol_token_account` and the final
`balance` required. Must be mutable as it will be closed at the end of the instruction.
