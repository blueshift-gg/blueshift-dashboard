...
Instruksi `make` melakukan tiga tugas:

- Menginisialisasi catatan Escrow dan menyimpan semua ketentuan kesepakatan.
...
**Catatan**: Kita akan menggunakan tipe yang diperkenalkan dalam [Pengantar Pinocchio](/id/courses/introduction-to-pinocchio/pinocchio-accounts).
...
- seed: angka acak yang digunakan selama derivasi seed. Harus berupa u64
- receive: jumlah yang ingin diterima oleh pembuat. Harus berupa u64
- amount: jumlah yang ingin disetor oleh pembuat. Harus berupa u64

Kita akan memeriksa untuk memastikan `amount` tidak nol, karena itu tidak akan masuk akal untuk sebuah escrow.
...
Kita mulai dengan menginisialisasi akun yang diperlukan dalam implementasi `TryFrom`, setelah kita mendeserialkan baik `instruction_data` maupun `accounts`.

Untuk langkah ini, kita membuat akun `Escrow` menggunakan trait `ProgramAccount::init::<Escrow>` dari fungsi pembantu yang diperkenalkan dalam [Pengantar Pinocchio](/id/courses/introduction-to-pinocchio/pinocchio-accounts). Demikian juga, kita menginisialisasi akun Vault karena perlu dibuat baru:
...
Sekarang kita dapat fokus pada logika itu sendiri yang hanya akan mengisi akun escrow dan kemudian mentransfer token ke vault.
