...
- `token program`: Tài khoản chương trình SPL Token. Tài khoản này cần thiết để thực hiện các thao tác token như transfer và minting. Phải là `executable`.
...
Nhờ bản nâng cấp [`p-token`](https://solana.com/upgrades/p-token), chúng ta có thể batch các lần transfer token và minting trong một CPI duy nhất, thay vì thực hiện ba lần gọi CPI riêng biệt.

```rust
use pinocchio_token::instructions::{Batch, BatchState, MintTo, Transfer};

/// Số lượng tài khoản cho batch instruction.
const MAX_ACCOUNTS_LEN: usize =
    Transfer::MAX_ACCOUNTS_LEN * 2
    + MintTo::MAX_ACCOUNTS_LEN;

/// Độ dài instruction data cho batch instruction.
...
Bạn đủ khả năng hoàn thành phần còn lại, vì vậy tôi sẽ để việc triển khai cho bạn:
