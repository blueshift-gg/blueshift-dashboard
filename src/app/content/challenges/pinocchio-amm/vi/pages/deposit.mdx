...
Vì `p-token` đã live trên mainnet, chúng ta sẽ batch các CPI token trong instruction này. Ở đây bạn sẽ đưa hai transfer và một mint vào cùng `Batch` rồi kết thúc bằng `batch.invoke_signed(&[signer])?;` vì LP mint cần `config` PDA ký.
...
- `token program`: Tài khoản chương trình SPL Token. `p-token` đã live trên mainnet và giữ nguyên program ID, nên tài khoản này cũng hỗ trợ batching. Phải là `executable`.
...
- Chuyển số lượng token từ các tài khoản token của người dùng vào các vault và đúc số lượng LP token thích hợp vào tài khoản token của người dùng. Dùng một `Batch` để gộp hai transfer và một mint vào một CPI.

Đây là phần bạn nối batch. Chúng ta vẫn dùng token helpers, chỉ là ở dạng batched. `Transfer` và `MintTo` đều triển khai `IntoBatch`, nên không cần tự serialize. Vì authority của LP mint là `config` PDA, ta kết thúc bằng `batch.invoke_signed(&[signer])?;`:
