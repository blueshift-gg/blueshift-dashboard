...
Vì `p-token` đã live trên mainnet, chúng ta sẽ batch các CPI token trong instruction này. Bạn đã thấy pattern batch ở `deposit`; ở đây bạn sẽ đưa hai transfer và một burn vào cùng `Batch` rồi kết thúc bằng `batch.invoke_signed(&[signer])?;` vì transfer từ vault cần `config` PDA ký.
...
- Chuyển số lượng từ các vault đến tài khoản token của người dùng và burn số lượng LP token thích hợp từ tài khoản token của người dùng. Dùng một `Batch` để gộp hai transfer và một burn vào một CPI.
