...
由于 `p-token` 已经在主网上线，我们会把这些 token CPI 批量处理。这里会把两次转账和一次 mint 放入同一个 `Batch`，并以 `batch.invoke_signed(&[signer])?;` 结束，因为 LP mint 需要 `config` PDA 签名。
...
- `token program`：SPL 代币程序账户。`p-token` 已在主网上线并保留相同的程序 ID，该账户也支持 batching。必须是 `executable`。
...
- 将用户的代币账户中的金额转移到金库，并向用户的代币账户铸造相应数量的 LP 代币。使用单一 `Batch`，让两次转账和一次 mint 在单一 CPI 中完成。

这就是你把 batch 组装起来的部分。我们仍然使用 token helpers，只是改成 batched 形式。`Transfer` 和 `MintTo` 都实现了 `IntoBatch`，所以不需要手动序列化。由于 LP mint 的 authority 是 `config` PDA，所以最后使用 `batch.invoke_signed(&[signer])?;`：
