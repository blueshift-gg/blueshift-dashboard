...
由于 `p-token` 已经在主网上线，我们会把这些 token CPI 批量处理。你在 `deposit` 已经看过 batch 的做法；这里会把两次转账和一次 burn 放入同一个 `Batch`，并以 `batch.invoke_signed(&[signer])?;` 结束，因为 vault 转账需要 `config` PDA 签名。
...
- 将金额从金库转移到用户的代币账户，并从用户的代币账户中销毁相应数量的 LP 代币。使用单一 `Batch`，让两次转账和一次 burn 在单一 CPI 中完成。
