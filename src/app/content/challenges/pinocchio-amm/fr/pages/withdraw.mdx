...
- _Burn_ la quantité appropriée de `mint_lp` provenant de l'ATA de l'utilisateur.

Comme `p-token` est déjà en ligne sur mainnet, nous batchons les CPI token pour cette instruction. Tu as deja vu le pattern batch dans `deposit` ; ici, vous ajoutez deux transfers et un burn dans un seul `Batch`, puis vous terminez par `batch.invoke_signed(&[signer])?;` car les transferts depuis le vault nécessitent la signature de la PDA `config`.
...
- Transférez les montants des vaults vers les comptes de jetons de l'utilisateur et brûler le montant approprié de jetons LP du compte de jetons de l'utilisateur. Utilisez un seul `Batch` pour regrouper les deux transfers et le burn en un seul CPI.
