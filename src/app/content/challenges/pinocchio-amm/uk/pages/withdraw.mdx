...
Оскільки `p-token` вже на mainnet, ми batch-имо token CPI для цієї інструкції. Ти вже бачив batch-патерн у `deposit`; тут ти кладеш два transfer і один burn в один `Batch` та завершуєш `batch.invoke_signed(&[signer])?;`, бо перекази з vault потребують підпису PDA `config`.
...
- Перекажіть суми з сховищ на токен-рахунки користувача та спаліть відповідну кількість LP токенів з токен-рахунку користувача. Використай один `Batch`, щоб обидва transfer і burn виконалися в одному CPI.
