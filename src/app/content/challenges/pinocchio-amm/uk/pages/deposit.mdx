...
Оскільки `p-token` вже на mainnet, ми batch-имо token CPI для цієї інструкції. Тут ти кладеш два transfer і один mint в один `Batch` та завершуєш `batch.invoke_signed(&[signer])?;`, бо LP mint потребує підпису PDA `config`.
...
- `token program`: Рахунок програми SPL Token. `p-token` вже на mainnet і зберігає той самий program ID, тож цей рахунок також підтримує batching. Має бути `executable`.
...
- Переказати суми з токен-рахунків користувача до сховищ і емітувати відповідну кількість LP токенів на токен-рахунок користувача. Використай один `Batch`, щоб обидва transfer і mint виконалися в одному CPI.

Саме тут ти збираєш batch. Ми все ще використовуємо token helpers, просто в batched-формі. `Transfer` і `MintTo` реалізують `IntoBatch`, тому нічого вручну серіалізувати не треба. Оскільки authority для LP mint — це PDA `config`, завершуємо `batch.invoke_signed(&[signer])?;`:
