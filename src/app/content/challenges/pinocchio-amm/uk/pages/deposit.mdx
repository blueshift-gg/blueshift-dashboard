...
- `token program`: Рахунок програми SPL Token. Він потрібен для виконання токен-операцій, таких як transfer і minting. Має бути `executable`.
...
Завдяки оновленню [`p-token`](https://solana.com/upgrades/p-token) ми можемо batch-ити token transfers і minting в одному CPI, замість того щоб робити три окремі CPI-виклики.

```rust
use pinocchio_token::instructions::{Batch, BatchState, MintTo, Transfer};

/// Кількість акаунтів для batch-інструкції.
const MAX_ACCOUNTS_LEN: usize =
    Transfer::MAX_ACCOUNTS_LEN * 2
    + MintTo::MAX_ACCOUNTS_LEN;

/// Довжина instruction data для batch-інструкції.
...
Ви повинні бути достатньо кваліфікованими, щоб зробити решту самостійно, тому я залишаю реалізацію вам:
