...
- `token program`：SPL 代幣程式帳戶。它用於執行轉帳和鑄造等代幣操作。必須是 `executable`。
...
透過 [`p-token`](https://solana.com/upgrades/p-token) 升級，我們可以把代幣轉帳和鑄造合併到單一 CPI 中，而不是執行三次獨立的 CPI 呼叫。
...
你應該已經足夠熟練可以自行完成其餘部分，所以我將實現的工作留給你：
