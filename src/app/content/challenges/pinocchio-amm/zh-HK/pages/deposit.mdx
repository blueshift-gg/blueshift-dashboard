...
由於 `p-token` 已經在主網上線，我們會把這些 token CPI 批次化處理。這裡會把兩次轉帳與一次 mint 加進同一個 `Batch`，並用 `batch.invoke_signed(&[signer])?;` 結束，因為 LP mint 需要 `config` PDA 簽名。
...
- `token program`：SPL 代幣程式帳戶。`p-token` 已在主網上線並保留相同的程式 ID，這個帳戶也支援 batching。必須是 `executable`。
...
- 將用戶的代幣帳戶中的金額轉移到保管庫，並鑄造相應數量的 LP 代幣到用戶的代幣帳戶。使用單一 `Batch`，讓兩次轉帳與一次 mint 在單一 CPI 中完成。

這就是你把 batch 接起來的部分。我們仍然使用 token helpers，只是改成 batched 形式。`Transfer` 和 `MintTo` 都實作了 `IntoBatch`，所以不需要手動序列化。因為 LP mint 的 authority 是 `config` PDA，所以最後使用 `batch.invoke_signed(&[signer])?;`：
