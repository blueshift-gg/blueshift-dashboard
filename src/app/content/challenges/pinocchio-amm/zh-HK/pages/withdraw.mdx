...
由於 `p-token` 已經在主網上線，我們會把這些 token CPI 批次化處理。你在 `deposit` 已經看過 batch 的做法；這裡會把兩次轉帳與一次 burn 加進同一個 `Batch`，並用 `batch.invoke_signed(&[signer])?;` 結束，因為 vault 轉帳需要 `config` PDA 簽名。
...
- 將金額從保管庫轉移到用戶的代幣帳戶，並從用戶的代幣帳戶中銷毀相應數量的 LP 代幣。使用單一 `Batch`，讓兩次轉帳與一次 burn 在單一 CPI 中完成。
