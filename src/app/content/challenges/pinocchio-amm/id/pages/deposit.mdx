...
Karena `p-token` sudah live di mainnet, kita akan membatch CPI token untuk instruksi ini. Di sini Anda akan memasukkan dua transfer dan satu mint ke dalam satu `Batch` lalu mengakhiri dengan `batch.invoke_signed(&[signer])?;` karena LP mint memerlukan tanda tangan PDA `config`.
...
- `token program`: Akun program SPL Token. `p-token` sudah live di mainnet dan mempertahankan program ID yang sama, sehingga akun ini juga mendukung batching. Harus `executable`.
...
- Transfer jumlah dari akun token pengguna ke vault dan mint jumlah token LP yang sesuai ke akun token pengguna. Gunakan satu `Batch` agar kedua transfer dan satu mint berjalan dalam satu CPI.

Di sinilah kamu merangkai batch. Kita tetap menggunakan token helpers, hanya saja dalam bentuk batched. `Transfer` dan `MintTo` mengimplementasikan `IntoBatch`, jadi tidak perlu serialisasi manual. Karena authority LP mint adalah PDA `config`, akhiri dengan `batch.invoke_signed(&[signer])?;`:
