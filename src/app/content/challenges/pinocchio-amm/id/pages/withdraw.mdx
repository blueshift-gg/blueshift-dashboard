...
Karena `p-token` sudah live di mainnet, kita akan membatch CPI token untuk instruksi ini. Kamu sudah melihat pattern batch di `deposit`; di sini Anda akan memasukkan dua transfer dan satu burn ke dalam satu `Batch` lalu mengakhiri dengan `batch.invoke_signed(&[signer])?;` karena transfer dari vault memerlukan tanda tangan PDA `config`.
...
- Transfer jumlah dari vault ke akun token pengguna dan bakar jumlah token LP yang sesuai dari akun token pengguna. Gunakan satu `Batch` agar kedua transfer dan satu burn berjalan dalam satu CPI.
