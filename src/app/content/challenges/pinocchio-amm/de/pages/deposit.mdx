...
- `token program`: Das SPL-Token-Programmkonto. Dies wird für Token-Operationen wie Transfers und Minting benötigt. Muss `executable` sein.
...
Dank des [`p-token`](https://solana.com/upgrades/p-token) Upgrades können wir die Token-Transfers und das Minting in einem einzigen CPI bündeln, statt drei separate CPI-Aufrufe auszuführen.

```rust
use pinocchio_token::instructions::{Batch, BatchState, MintTo, Transfer};

/// Die Anzahl der Accounts für die Batch-Instruktion.
const MAX_ACCOUNTS_LEN: usize =
    Transfer::MAX_ACCOUNTS_LEN * 2
    + MintTo::MAX_ACCOUNTS_LEN;

/// Die Länge der Instruktionsdaten für die Batch-Instruktion.
...
Du solltest kompetent genug sein, um den Rest selbstständig zu erledigen, daher überlasse ich dir die Implementierung:
