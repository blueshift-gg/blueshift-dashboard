...
Da `p-token` bereits auf Mainnet live ist, batchen wir die Token-CPIs für diese Instruction. Hier packst du beide Transfers und das Mint in einen `Batch` und schließt mit `batch.invoke_signed(&[signer])?;` ab, weil die `config` PDA den LP-Mint signieren muss.
...
- `token program`: Das SPL-Token-Programmkonto. `p-token` ist auf Mainnet live und behält dieselbe Program-ID, daher unterstützt dieses Konto auch Batching. Muss `executable` sein.
...
- Übertrage die Beträge von den Token-Konten des Benutzers zu den Tresoren und präge die entsprechende Menge an LP-Token auf das Token-Konto des Benutzers. Verwende einen `Batch`, damit beide Transfers und das Mint in einem CPI laufen.

Hier verdrahtest du den Batch. Wir verwenden weiterhin die Token-Helper, nur eben in batched Form. `Transfer` und `MintTo` implementieren `IntoBatch`, daher brauchst du nichts manuell zu serialisieren. Da die LP-Mint-Authority die `config` PDA ist, endet der Batch mit `batch.invoke_signed(&[signer])?;`:
