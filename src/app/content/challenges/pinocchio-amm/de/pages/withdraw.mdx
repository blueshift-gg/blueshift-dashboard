...
Da `p-token` bereits auf Mainnet live ist, batchen wir die Token-CPIs für diese Instruction. Du kennst das Batch-Pattern aus `deposit`; hier packst du beide Transfers und das Burn in einen `Batch` und schließt mit `batch.invoke_signed(&[signer])?;` ab, weil die `config` PDA die Vault-Transfers signieren muss.
...
- Übertrage die Beträge aus den Tresoren auf die Token-Konten des Benutzers und verbrenne die entsprechende Menge an LP-Tokens vom Token-Konto des Benutzers. Verwende einen `Batch`, damit beide Transfers und das Burn in einem CPI laufen.
