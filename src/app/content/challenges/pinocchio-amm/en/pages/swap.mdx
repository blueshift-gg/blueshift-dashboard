...
- `token program`: The SPL Token program account. This is required to perform token operations such as transfers and minting. With `p-token` live on mainnet, this same token program account also supports batching. Must be `executable`.
...
- Create the transfer logic checking the `is_x` value. By now you already know the batch pattern from `deposit`, so queue both transfers into a `Batch` and invoke the token program once. This swap uses `batch.invoke_signed(&[signer])?;` because the `config` PDA must sign the vault transfer.
