...
- Batch the token transfers into a single CPI. You already saw the batch pattern in `deposit`, so here we do the same with the two `Transfer` instructions and invoke the token program once. Since the transfer from the vault still requires the `config` PDA to sign, the batch should end with `batch.invoke_signed(&[signer])?;`.
