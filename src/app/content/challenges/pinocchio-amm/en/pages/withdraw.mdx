...
With `p-token` now live on mainnet, we batch the token CPIs for this instruction. By now you already know the batch pattern from `deposit`, so queue the transfers and burn into a single `Batch`, then finish with `batch.invoke_signed(&[signer])?` because the `config` PDA must authorize vault transfers.
...
- Transfer the amounts from the vaults to the token accounts of the user and burn the appropriate amount of LP tokens from the user token account. Use a single `Batch` so both transfers and the burn run in one CPI.
