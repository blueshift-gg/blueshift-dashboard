...
Thanks to the [`p-token`](https://solana.com/upgrades/p-token) upgrade, we can batch the token transfers and LP tokens minting in one CPI, instead of doing three separate CPI calls.
