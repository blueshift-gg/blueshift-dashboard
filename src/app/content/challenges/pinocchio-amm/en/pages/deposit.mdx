...
With `p-token` live on mainnet, we batch the token CPIs for this instruction. Queue the transfers and mint into a single `Batch`, then finish with `batch.invoke_signed(&[signer])?` because the `config` PDA must authorize the LP mint.
...
- `token program`: The SPL Token program account. This is required to perform token operations such as transfers and minting. With `p-token` live on mainnet, this same token program account also supports the batch flow. Must be `executable`.
...
- Transfer the amounts from the token accounts of the user to the vaults and mint the appropriate amount of LP tokens to the user token account. Use a single `Batch` so both transfers and the mint run in one CPI.

This is the part where you wire the batch. We still use the token helpers, just in batched form. `Transfer` and `MintTo` both implement `IntoBatch`, so we don't need to manually serialize anything. Because the LP mint authority is the `config` PDA, we end with `batch.invoke_signed(&[signer])?;`:
