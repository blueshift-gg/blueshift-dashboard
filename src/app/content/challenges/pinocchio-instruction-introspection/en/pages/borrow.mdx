...
The `loan` instruction is the first half of our flash loan system. It performs three critical steps to ensure safe and atomic lending:
1. Deserialize a dynamic amount of accounts based on the number of loan that the user wants to take.
2. Save all these loans in the `loan` "scrap" account and calculate the final balance that the `protocol_token_account` needs to have.
...
4. Transfer funds: Move all the requested `borrow_amount` from the protocol's treasury to the borrower's account
...
- `loan`: the "scrap" account used to save the `protocol_token_account` and the final `balance` that they need to have. Must be mutable
...
As you can see, since the `token_accounts` are a dynamic number of accounts, we pass it in similarly to how we pass `remaining_accounts`.

But to make sure that the structure is correct, we need to add some validation to it; each loan needs a `protocol_token_account` and a `borrower_token_account` for this reason we need to make sure that there are some accounts in that array and that the number of accounts is divisible by 2

Additionally, we verify that the loan account contains no existing data, as pre-existing data would create a critical vulnerability in our program. 

We also check for duplicate `protocol_token_account` entries, which would allow an attacker to request multiple loans while only being required to repay the largest amount.
...
We begin by performing a check to make sure that the number of `amounts` is equal to the number of `token_accounts` divided by two to make sure that we have all the accounts or amount to perform multiple loans, after we've deserialized both the `instruction_data` and the `accounts`.
...
We then proceed to create the `signer_seeds` needed for transferring out the tokens to the borrower and creating a `loan` account that can contain all the loans we're taking in this instruction by using `size_of::<LoanData>() * self.instruction_data.amounts.len()`
...
We then proceed to create an empty slice in the `loan_data` that we're going to populate in the for cycle while performing all transfers for each loan:
...
We can finally create a `for` loop that goes through all the loans, grab the `protocol_token_account` and the `borrower_token_account` and calculate the `balance` due to the protocol before saving that data in the `loan` account and transferring out the tokens.
...
We finish by performing the checks needed using instruction introspection. We're going to just check if the last instruction of the transaction is a `repay` instruction and if the `loan` account used there is the same as the one that we're using in the `loan` instruction.
