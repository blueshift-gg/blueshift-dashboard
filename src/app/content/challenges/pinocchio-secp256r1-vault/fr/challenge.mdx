...
Bien que _unwrap_ soit généralement déconseillé en production, dans ce cas précis il est utilisé en toute sécurité car nous avons déjà validé la taille des données dans la méthode `try_from`. Si la taille des données n'est pas correcte, une erreur sera renvoyée avant d'atteindre ce point.
