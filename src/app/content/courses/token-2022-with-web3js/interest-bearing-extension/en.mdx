...
To initialize the `InterestBearing` extension on a `Mint` account we're going to need the `interestBearingMint()` function.
...
<ArticleSection
  name="Updating the Interest Bearing Extension"
