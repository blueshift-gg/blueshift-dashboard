...
Accounts are Solana's fundamental data structure. Understanding their structure, ownership rules, and rent mechanism is essential whether you're building programs or using applications.

Every Solana account has the same basic structure, follows the same ownership rules, and requires the same minimum balance to persist.
...
While all accounts share the same structure, they serve different purposes based on their owner and executable flag.
...
Accounts require a minimum lamport balance to stay alive. The minimum balance depends on account size and is refundable when the account closes.
...
These values can change through governance, but they remain relatively stable.

**Calculating rent for your account:**
...
**Rent-exempt balance:** Once an account has the minimum balance for its size, it becomes "rent-exempt." No further rent will be charged. The account persists indefinitely as long as it maintains this minimum.
...
**Why this matters:** Creating accounts costs SOL. When you mint an NFT, create a token account, or initialize program state, you pay the rent-exempt minimum. These costs are small but accumulate. Applications with thousands of accounts need to account for total rent costs.

<ArticleSection name="Ownership Rules" id="ownership-rules" level="h2" />

Only the owner can modify the account. Ownership defines what operations are possible on an account.
...
Unlike databases where you insert records, Solana accounts must be explicitly created and funded before use.
...
Programs interact with accounts by reading and writing their data fields. Programs serialize and deserialize account data using this pattern:
...
This pattern enables programs to work with structured data while storing it as raw bytes on-chain.

<ArticleSection name="Associated Token Accounts" id="associated-token-accounts" level="h2" />

Token accounts use a special pattern called Associated Token Accounts (ATAs). Each user can have multiple token accounts—one for USDC, one for BONK, one for each SPL token they hold.

Finding a user's token account for a specific token mint could be difficult without a standard. Associated Token Accounts solve this with a deterministic address derivation:
...
Given a wallet address and token mint address, there's exactly one associated token account address. Benefits:
- Wallets automatically find your USDC account
- Programs know where to send your tokens
...
All account data is public. You can view any account using block explorers.

**Solana Explorer** (https://explorer.solana.com):
- Enter any account address
- View lamport balance
- See account owner
- Inspect raw data (as bytes or decoded if format is known)
- Track transaction history

**Solscan** (https://solscan.io):
- Enhanced UI with better data visualization
- Token holdings listed clearly
- NFT metadata displayed with images
- More user-friendly for non-technical users

Try viewing a well-known account like the USDC token mint: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
...
This transparency is fundamental to blockchain. Anyone can verify any data at any time without requiring permission.

The next lesson covers programs and transactions.
