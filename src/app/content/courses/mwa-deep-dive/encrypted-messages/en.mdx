...
GCM is an _authenticated encryption with associated data_ (AEAD) mode. It's efficient, widely supported in hardware, and available in the Web Crypto API.
...
async function decryptMessage(
  ...
  if (receivedSeq !== expectedSequenceNumber) {
    throw new Error(
      `Sequence number mismatch: expected ${expectedSequenceNumber}, got ${receivedSeq}`,
    );
  }
...
export async function encryptJsonRpcMessage(
  ...
  return new Uint8Array([...new Uint8Array(seqNumBuffer), ...iv, ...new Uint8Array(ciphertext)]);
}
```
