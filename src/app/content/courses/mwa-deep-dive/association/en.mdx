...
Before a session can exist, the wallet needs to know where to connect and how to verify the dApp's identity. This is the _association_ phase: the dApp communicates connection parameters to the wallet through a URI.
...
- `association`: Base64url-encoded P-256 public key (the _association public key_)
...
Association establishes _which_ session is happening, not _who_ is participating.
