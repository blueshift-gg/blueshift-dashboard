...
Esta seção do artigo foi inspirada no [antigo site do Richard](https://web.archive.org/web/20230107121438/https://bpf.wtf/sol-0x04-syscalls/).
Syscalls permitem que programas em execução na máquina virtual da Solana interajam com recursos externos. Elas podem ser invocadas usando a instrução `call` com o valor imediato definido como o hash do símbolo.

<ArticleSection name="Syscalls" id="syscalls" level="h2" />

Parâmetros de entrada são fornecidos nos registradores `r1`, `r2`, `r3`, `r4`, `r5`.
O registrador `r0` é sempre definido, seja para o valor de retorno, ou zero para syscalls void.
Outros registradores são preservados. Syscalls não alocam um call frame.

Se o chamador fornecer entradas inválidas que causam um erro de runtime (ex: chamar `sol_memcpy_` em um ponteiro nulo), a VM levanta o erro no local da chamada.

| Syscall                                 | Hash         | `r1`                          | `r2`                            | `r3`                         | `r4`                            | `r5`                         | Retorna |
| --------------------------------------- | ------------ | ----------------------------- | ------------------------------- | ---------------------------- | ------------------------------- | ---------------------------- | ------- |
...
### DEPRECATED - use `sol_get_sysvar` em vez disso

| Syscall                         | Hash         | `r1`            | Retorna |
| ------------------------------- | ------------ | --------------- | ------- |
...
**Nota:** `sol_get_fees_sysvar` não pode passar por `sol_get_sysvar` mas pode ser removido assim que não estiver mais em uso

### JIT Intrinsic

Você pode encontrá-los nas páginas de pesquisa [Accelerating the SVM with JIT Intrinsics ](https://blueshift.gg/research/accelerating-svm-with-jit-intrinsics) e [Accelerating u128 Math with Libcalls and JIT Intrinsics](https://blueshift.gg/research/accelerating-u128-math-with-libcalls-and-jit-intrinsics)

| Syscall            | Hash         | `r1`                | `r2`     | `r3`     | Retorna |
| ------------------ | ------------ | ------------------- | -------- | -------- | ------- |
