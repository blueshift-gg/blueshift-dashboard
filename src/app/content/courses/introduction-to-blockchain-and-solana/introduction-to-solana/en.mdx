...
Other blockchain like Ethereum, follows a more familiar object-oriented model:
...
For Solana, instead of contracts owning data, everything is an account, and accounts are just data containers that can be owned by programs.

Think of it like this:
- Accounts: Database records/files in a filesystem
...
Every Solana account shares the same base Account type, regardless of their purpose:
...
> Important: This signing capability only exists when the account is owned by the "System Program" (Solana's base program). Once ownership transfers to another program, that program gains complete control over the account's data and lamports, regardless of whether you hold the private key.

We said before that the addresses are the key to locate the corresponding on-chain data of the account. The account address acts as the "unique ID" for each entry in the "Accounts" table.

### Key Account Properties

- Accounts can store up to 10MiB of data, which can contain either: executable program code (for program accounts); program state and user data (for data accounts)
...
2. Token Accounts: Store SPL tokens information like ownership or token itself and are owned by the Token Program
3. Program Accounts: Store executable code (or the Smart Contract) and are marked as `executable: true`
...
Other blockchain like Ethereum, follows a simpler model where each transaction calls one function:
...
The main difference is that to create transactions on Ethereum you have to:
- Make multiple separate transactions (expensive and slow)
- Build everything into one smart contract function
- Use complex proxy patterns

Solana instead, follows an atomic aransaction model; that measn that every transaction can contain multiple instructions, and they all succeed or fail together; atomically.
...
Solana transactions include instructions that invoke programs on the network. As said before, we can have multiple instructions in a transaction and transaction are atomic: if any instruction fails, the entire transaction fails and no changes occur; this instructions execute in sequential order.

The transaction size limit is 1232 bytes and each instruction requires three pieces of information: the address of the program to invoke; the accounts the instruction reads from or writes to; any extra data required by the instruction (e.g., function arguments)
...
The base fee for a transaction is 5000 lamports per signature on the transaction and the prioritization fee (optional) is an extra fee you pay to the validator to increase the chance that the current leader processes your transaction.

The prioritization fee equals: compute unit limit (maximum compute units your transaction) * compute unit price (price per compute unit, in micro-lamports that we want to pay).
...
Other blockchain like Ethereum, follows an object-oriented model where smart contracts are like classes that own their data:
...
Solana programs are stateless; they don't own any data and operate only on accounts passed to them.
...
We've already explored `Accounts` on Solana extensively in the previous section. But we haven't discussed the "superpower" that accounts can possess: Program Derived Addresses (or PDAs).
...
The only seed of this PDA is the `signer` publickey and the result is an address that we can use to create hashmap-like structures on-chain from a predefined set of inputs (e.g. strings, numbers, and other account addresses).
...
Another superpower that PDA unlock is program signing. Since PDAs don't have private keys, how can they authorize transactions? Solana's runtime allows programs to "sign" for PDAs that are derived from the program's address.
...
Solana instead allows programs to call other programs within the same transaction using CPIs. Think of it like calling multiple microservices in a single atomic database transaction that makes possible to create "synchronous" calls across different services and permits composability without integrations.
...
- Signer privileges from the original transaction extend to called programs (ex. A->B)
- Programs can make CPIs up to a depth of 4 levels deep
