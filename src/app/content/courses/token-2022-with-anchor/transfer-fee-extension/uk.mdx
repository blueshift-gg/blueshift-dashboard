...
Щоб переконатися, що одержувач комісії не отримує блокування запису щоразу, коли хтось виконує обмін, і щоб забезпечити можливість паралельного виконання транзакцій, що містять `Mint` з цим розширенням, комісія відкладається на рахунку токенів одержувача, з якого може зняти кошти лише `Withdraw Authority`.
...
Оскільки `Anchor` не має макросів для розширення `transfer_fee`, ми створимо обліковий запис `Mint` за допомогою необроблених CPI.

Ось як створити мінт з розширенням комісії за переказ:
...
<ArticleSection
  name="Transferring Tokens with the Fee"
