...
Die `TransferFee` Extension ist eine `Mint` Extension, die es dem Ersteller ermöglicht, eine "Steuer" auf den Token festzulegen, die bei jedem Tausch erhoben wird.

Um sicherzustellen, dass der Gebührenempfänger nicht bei jedem Tausch schreibgeschützt wird und um zu gewährleisten, dass wir Transaktionen mit einer `Mint` mit dieser Extension parallelisieren können, wird die Gebühr im Token-Konto des Empfängers zurückgelegt, auf das nur der `Withdraw Authority` zugreifen kann.

<ArticleSection
  name="Initialisierung des Mint-Kontos"
  id="initializing-the-mint-account"
  level="h2"
/>

Da `Anchor` keine Makros für die `transfer_fee` Extension hat, werden wir ein `Mint` Konto mit den rohen CPIs erstellen.

So erstellt man einen Mint mit der Transfer Fee Extension:
...
<ArticleSection
  name="Übertragung von Tokens mit der Gebühr"
  id="transferring-tokens-with-the-fee"
  level="h2"
/>

Um Tokens für `Mint` zu übertragen, die eine TransferFee` Extension haben, gibt es zwei Wege:

- Wir können die normale `transfer_checked()` Anweisung verwenden, wobei die Berechnung der Gebühr automatisch erfolgt
- Wir können die `transfer_checked_with_fee()` Anweisung verwenden und manuell die `fee` angeben, die wir bei dieser Übertragung zahlen werden. Dies ist sehr nützlich, wenn wir sicherstellen wollen, nicht "gerugged" zu werden, falls die Autorität die Gebühr ändert und eine ungewöhnlich hohe Gebühr festlegt; es ist wie das Festlegen des Slippages für eine Übertragung.

> Selbst wenn die Autorität die Gebühr ändert, wird die neue Gebühr erst 2 Epochen nach der Festlegung aktiv

So erstellt man eine Übertragung mit der `transfer_checked_with_fee()` Anweisung:
...
Um dann die Gebühren abzuheben, können wir alle `Token Account` sammeln, die Gebühren "gefangen" haben und sie als verbleibende Konten übergeben. Auf der Typescript-Seite würde es etwa so aussehen:
...
Nachdem wir alle benötigten Konten eingerichtet haben, müssen wir zum Abheben von Tokens auf das `authority_token_account` zunächst alle Gebühren von Token-Konten mit der Anweisung `harvest_withheld_tokens_to_mint()` "ernten" und sie dann an ein vom `authority` gewähltes `token_account` mittels der Anweisung `withdraw_withheld_tokens_from_mint()` überweisen.
...
<ArticleSection name="Aktualisierung der Gebühr" id="updating-the-fee" level="h2" />

Nachdem wir unser `Mint` mit der Erweiterung `TranferFee` initialisiert haben, müssen wir diese spezifische Gebühr möglicherweise in Zukunft aktualisieren. Um sicherzustellen, dass der Ersteller seine Token-Inhaber nicht mit einem "Köder und Austausch" betrügt, indem er die Gebühr bei jeder Übertragung sehr hoch ansetzt, wird die neue `TranferFee` erst nach 2 Epochen aktiviert.
...
Um die Gebühr zu ändern, können wir die Anweisung `transfer_fee_set()` wie folgt verwenden:
