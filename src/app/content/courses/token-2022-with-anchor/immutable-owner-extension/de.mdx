...
Die `ImmutableOwner` Erweiterung ist eine `Token` Kontoerweiterung, die jegliche Änderungen an der Eigentümerschaft des `Token` Kontos verhindert. Dies schützt Konten vor unbefugtem Zugriff und Übertragungsversuchen.
...
Da `Anchor` keine Makros für die `immutable_owner` Erweiterung hat, werden wir ein `Mint` Konto mit den rohen CPIs erstellen.

So erstellt man eine Mint mit der Immutable Owner-Erweiterung:
