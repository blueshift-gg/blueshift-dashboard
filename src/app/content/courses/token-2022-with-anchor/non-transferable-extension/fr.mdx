...
L'extension `non_transferable` (Non transférable) est une extension de compte de `Mint` qui empêche le transfert de jetons entre comptes, les rendant ainsi liés de manière permanente à leurs détenteurs actuels.
...
Comme `Anchor` ne possède aucune macro pour l'extension `non_transferable` nous allons créer un compte de `Token` à l'aide de CPIs bruts.

Voici comment créer un compte de mint avec l'extension `non_transferable` :
