...
L'extension `interest_bearing` (Porteur d'Intérêts) est une extension de compte de `Mint` qui permet aux utilisateurs d'appliquer un taux d'intérêt à leurs jetons et de récupérer le total mis à jour, intérêts compris, à tout moment.
...
Comme `Anchor` ne possède aucune macro pour l'extension `interest_bearing` nous allons créer un compte de `Mint` à l'aide de CPIs bruts.
...
<ArticleSection
  name="Mettre à Jour l'Extension Interest Bearing"
