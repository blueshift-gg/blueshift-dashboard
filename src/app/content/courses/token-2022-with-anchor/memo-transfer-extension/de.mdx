...
# Die Memo-Transfer-Erweiterung

Die `MemoTranfer` Erweiterung ist eine `Token` Kontoerweiterung, die sicherstellt, dass alle eingehenden Überweisungen auf ein Token-Konto ein Memo enthalten, was eine verbesserte Transaktionsverfolgung und Benutzeridentifikation ermöglicht.

<ArticleSection
  name="Initialisierung des Token-Kontos"
  id="initializing-the-token-account"
  level="h2"
/>

Da `Anchor` keine Makros für die `memo_transfer` Erweiterung hat, werden wir ein `Token` Konto mit den direkten CPIs erstellen.

Hier ist, wie man einen Token mit der Memo-Transfer-Erweiterung erstellt:
...
<ArticleSection
  name="Token mit einem Memo übertragen"
  id="transferring-token-with-a-memo"
  level="h2"
/>

Um das Memo-Programm auf Solana zu nutzen, haben wir zwei mögliche Wege.
...
<ArticleSection
  name="Disabling and Enabling the Memo"
