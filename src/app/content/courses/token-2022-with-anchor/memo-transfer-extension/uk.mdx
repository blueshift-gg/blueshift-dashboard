...
Розширення `MemoTranfer` є розширенням облікового запису `Token`, яке забезпечує, щоб усі вхідні перекази на токен-рахунок містили примітку, що сприяє покращеному відстеженню транзакцій та ідентифікації користувачів.

<ArticleSection
  name="Initializing the Token Account"
  id="initializing-the-token-account"
  level="h2"
/>

Оскільки `Anchor` не має макросів для розширення `memo_transfer`, ми створимо обліковий запис `Token` за допомогою необроблених CPI.
...
<ArticleSection
  name="Transferring Token with a Memo"
