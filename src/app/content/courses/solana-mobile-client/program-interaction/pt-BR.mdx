...
// Configuração global
const [globalConfig] = PublicKey.findProgramAddressSync([Buffer.from("config")], programId);
...
<ArticleSection
  name="Trabalhando com Programas Anchor"
  id="working-with-anchor-programs"
  level="h2"
/>
...
const fullData = Buffer.concat([discriminator, borsh.serialize(argsSchema, args)]);
```

<ArticleSection
  name="Transações com Múltiplas Instruções"
  id="multi-instruction-transactions"
  level="h2"
/>
...
```typescript
async function createAndInitialize(connection: Connection, userWallet: PublicKey): Promise<string> {
...
```typescript
async function initializeUserAccount(program: Program, userWallet: PublicKey): Promise<string> {
...
try {
  ...
  if (error.message.includes("Transaction simulation failed")) {
    ...
    const programError = logs.find((log) => log.includes("Error") || log.includes("error"));
