...
> Sua aplicação não envia a transação para a rede, então você não terá acesso à assinatura da transação para fins de rastreamento. Use parâmetros de referência como explicado na [Lição de Introdução](/pt-BR/courses/solana-pay/introduction#reference)
...
const hasRequiredNFT = nfts.some(
  (nft) => nft.collection?.address.toString() === requiredCollection,
);
